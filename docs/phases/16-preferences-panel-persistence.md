...
`Switch`'s `onChange` commits directly back into `AppState.preferences` via
...
`PreferencesDisplay` itself has 9 mutually independent toggles (§5.8's table) plus the
...
flat view-model hook. `usePreferencesDisplayDomain.ts` holds the pure per-toggle update
functions (e.g. given the current preferences object and a key, return a new preferences
object with that key flipped/changed — no React, no telescope imports).
...
holds one event-handler closure per `Switch` (each currying a domain function with the
...
- The drawer renders exactly 9 `Switch` rows, in §5.8's table order, each labeled with the table's exact label text and bound to the correct preference key: Piece Type: Shapes or Faces → `pieceType`, Hint Fit Piece Count → `hintFitPieceCount`, Hint Fit Piece Unique Cell → `hintAvailablePieceUniqueCell`, Hint Piece Cells → `hintPieceCells`, Hint Fit On Drag → `hintFitOnDrag`, Show Fit Pieces on Hover → `showFitPiecesOnHover`, Prevent Invalid Moves → `preventInvalidMoves`, Hint Game Is Solvable → `hintGameIsSolvable`, Sound → `sound`.
- Every one of the 9 toggles is mutually independent (flipping one never changes another) and commits its new value into `AppState.preferences` via a magnified telescope, not via prop-drilled callbacks.
...
- With devtools, write a partial/older-shaped preferences object into `localStorage` (e.g. only `{ "sound": false }`, omitting every other key) and reload; confirm the app loads without crashing, the Preferences drawer shows `Sound` off and every other toggle at its correct default value, and no console errors appear.
